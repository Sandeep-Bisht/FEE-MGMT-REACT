...
var counter =0
var FinalDefaulter=[]
class DefaulterMaker extends React.Component{
    constructor(props){
        super(props)
        this.state={
          AllDefaulter:[],
...
this.setStudentFeeStructure() 
            ...
            if( !JSON.stringify(studentArray).includes(item.admission_no+item.account_no+item.class_name)){
              studentArray.push({"paid_upto_month_status":false,"class_name":item.class_name,"section":item.section,"name":item.student.name,"admission_no":admission_no,"account_no":item.account_no,"fee_concession":item.fee_concession,"paid_upto_month":item.student.paid_upto_month,"student_balance":0,"unique_key":item.admission_no+item.account_no+item.class_name})
              this.setState({studentArray:studentArray})
  }
  this.setStudentFeeStructure() 
...
if(item.admission_no=="18996"){
alert("18996 "+item.paid_upto_month)
}
...
paidFees.push({"session":this.state.session,"class_name":item.class_name,"section":item.section,"monthly":total_monthly_fee,"annual_fees":annual_fees,"admission_no":item.admission_no,"name":item.name,"account_no":item.account_no,"student_balance":student_balance,"TotalPreviousBalance":annual_fees+total_monthly_fee+(parseInt(student_balance)*-1)})
// console.log("this is paid annual Fees "+annual_fees)
  ...
  if(paidFees.length==this.state.AllStudent.length)
  {
    // alert("paid fees length "+paidFees.length)
    // this.PushData()
  }
 }
 ...
 PushData(){
  ...
  const url = "http://144:91:110:210:4800/ImportallDefaulter"
  fetch(url, {
      // headers : { 
      //   'Content-Type':'application/json',
      //   'Accept':'application/json'
    ...
    render(){
      ...
      count =0
        ...
        // })}
          ...
          const csvData = [
            ["ADMISSSION NO", "ACCOUNT NO", "STUDENT" ,"CLASS" ,"ONE TIME", "ANNUAL" ,"MONTHLY","FINE","TOTAL"],
          ];
          
          {this.state.paidFees.map((item,ind)=>{
            if(item.annual_fees+item.monthly+parseInt(item.student_balance)*-1 >0){
            csvData.push( [item.admission_no,item.account_no,item.name,item.class_name,item.one_time_fees,item.annual_fees,item.monthly,0,item.annual_fees+item.monthly+parseInt(item.student_balance)])
            }
          // })}
          })
          }
            ...
            <>
              <div className= "row layoutCard">
                ...
                <div className="col-12 form-group">
                  <button className="btn btn-success" onClick={()=>{this.StudentStrenght()}}>Get Defaulter</button>
