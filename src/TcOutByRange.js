...
import $ from 'jquery'; 
var numberCount =1
class TcOutByRange extends React.Component{
    constructor(props){
        super(props)
        this.state={
            _id:'',
  ...
  getAllCertificateDetails = async() => {
    await console.log("wait wait")
    fetch("http://localhost:4800/getAllTransferCertificate"
    ,{
    method: 'POST',
    headers: {
      Accept: 'application/json',
        ...
        return(
            <>
             ...
             <div id="AllModalStudent" class="modal fade" role="dialog">
                ...
                <div className="col-12 text-center">
                  <h4 className="pb-5">TC STUDENTS</h4>
                </div>
              <div className="col-12 ">
                        <DataTable
                        data={data}
                      ...
                      <table class="table table-striped">
                        ...
                        {this.state.AllStudent.sort((a,b) => parseInt(a.admission_no) - parseInt(b.admission_no)).map((item,index)=>{
                              if(item.date_of_tc >= this.state.FromDateCopy && item.date_of_tc <= this.state.ToDateCopy ){ 
                            ...
                            return(
                              <tr>
                                ...
                                <td>{item.date_of_cheque?item.date_of_cheque!=undefined ?Moment(item.date_of_cheque).format("DD-MM-YYYY")!=''?Moment(item.date_of_cheque).format("DD-MM-YYYY"):null:null:null}</td>
                                <td>{item.result_remark}</td>
                              </tr>
                            )
                                  }
