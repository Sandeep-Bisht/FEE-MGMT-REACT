...
class FeeReceipt extends React.Component{
        ...
        .then(async (data) => {  
            ...
            if(data[0] !=undefined){
                ...
                if(parseInt(A.diff(B, 'days')) < 0){
                  ...
                  await this.setState({AllOldFees: data,last_session:data[data.length-1].session,last_fees_date:last_date}) 
                }
                else{
                  last_date=data[data.length-1].last_fee_date

                  this.setState({AllOldFees: data,last_session:data[data.length-1].session,last_fees_date:last_date,balance:data[data.length-1].balance}) 
    ...
    setBalance =async()=>{
        await console.log("wait")
        if(this.state.is_full_free_ship=="false" || this.state.is_full_free_ship==""){
        ...
        if(this.state.paid_amount=="0"){
            ...
            if(dateEnd.isAfter(dateStart, 'days') || dateStart.format('M') === dateEnd.format('M')){
                
                // this or condition remove from while || dateStart.format('M') === dateEnd.format('M') 
            while (dateEnd.isAfter(dateStart, 'days') || dateStart.format('M') === dateEnd.format('M') ) {  
        ...
        // this code for set All fees not paid to set 0
         ...
         if(paidAmount<=0){
            ...
            {this.setState({remaning_balance:remainbalance,months:shortmonths,feemonths:months,fromtomonths:fromtomonths,last_fee_date:last_fee_date,total_monthly_fee:monthlyfee,grand_total:this.state.paid_amount})}
            return false
         }
        while( paidAmount >0  ){            
            if( Moment(this.state.last_fees_date).format('M')=="3" && this.state.session == this.state.last_session && this.state.last_fee_status==true )  {
  ...
  // End GetAllStudentCount Api
    ...
    render(){
            ...
            <div id="ConfirmationModal" class="modal show" role="dialog" >
                <div class="modal-dialog">
                    ...
                    <div class="modal-header">
                        ...
                        <button type="button" class="close" onClick={()=>{this.setState({ConfModal:false})}}>&times;</button>
                    </div>
                    <div class="modal-body">
                        <input type="password" className="form-control" onKeyPress={(e)=>{if(e.key=="Enter"){this.DeleteReceipt(e.target.value)}}}/>
                    </div>
                    </div>
        ...
        <div class="modal-dialog modal-md">            
                            ...
                            <div className="row">
                             ...
                             <option value="">All Session</option>
                              {this.state.AllSession.map((item,index)=>{
                                ...
                                <div className="col-5">
                                    <strong>Admn No - </strong> <input id="FeeDetailAdmnNo"  value={this.state.admission_no} onFocus={(e)=>{this.handleFocusInput(e)}} onChange={(e)=>{this.setState({admission_no:e.target.value});this.searchByAdmission_no(e)}}  onKeyPress={(e)=>{ if(e.key === 'Enter'){this.searchByAdmission_no(e)}}}  /> <button type="button" onClick={()=>{this.getStudent()}} class="btn btn-success btn-sm" data-toggle="modal" data-target="#AllModalStudent">...</button><span> / <strong>{this.state.account_no}</strong></span><br/>
                            ...
                            if(item.session==this.state.session){
                                return(
                                            <tr>
                                                ...
                                                onClick={() => {if(window.confirm('Are You Sure?')){this.DeleteReceipt(item._id)};}}><i className="fa fa-trash" aria-hidden="true"></i></button></td>
                            ...
                            if(this.state.session==""){
                                return(
                                            <tr>
                                                ...
                                                data-dismiss="modal" data-toggle="modal"                      onClick={() => {if(window.confirm('Are You Sure?')){this.DeleteReceipt(item._id)};}}
                                                ><i className="fa fa-trash" aria-hidden="true"></i></button></td>
                                                {/* onClick={() => {this.setState({ConfModal:true,DeleteId:item._id})}} */}
                                            </tr>
                                )
                            }
                    ...
                    <div className="form-row">
                        ...
                        <div className="col-3 form-group">
                            <div className="form-row">
                                ...
                                <div className="col-8 form-group " id="focusguard-1">
                                    <input type="date" className="w-100" id="firstInput" value={this.state.receipt_date} onChange={(e)=>{this.setState({receipt_date:e.target.value.toUpperCase()})}} />
                        ...
                        <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#OldFeeModal">Fee Details</button> 
                        ...
                        {this.state.AllStudent.map((item,index)=>{
                            ...
                            <tr  className={(item.class_name=="12SCI" || item.class_name=="12COM")&& item.session!=this.state.session && item.student.tc_status==0? "bg-primary":item.student.tc_status==1 ? "bg-danger":item.student.tc_status=='sos'?"bg-primary":null}>
            ...
            <div className="row ReceiptLayoutCard feesLayoutCard" id="AnnualSection" >
            <div className="col-12">
                <div className="form-row">
                {/* <div className="col-9">
            ...
            <div className="col-12">
                <div className="form-row">
                <div className="col-12 text-center" >
                 <h3 className="receipt_h3 pb-5  "><u>Annual Fee</u><span style={{float:"right"}}><button className="btn btn-danger btn-sm mb-2" id="HideAnnual">- Annual Fee</button></span></h3>
                </div>
            ...
            <div className="col-12">
                    <div className="form-row">
                        <div className="col-12 text-center">
                            <h3 className="receipt_h3 pb-5 " ><u>Monthly Fee</u><span style={{float:"right"}}><button className="btn btn-success btn-sm " id="ShowAnnual">+ Annual Fee</button></span></h3>                            
                        </div>
                        {this.state.Allfees.map((item,index)=>{
                            if(item.fee_category==="MONTHLY"){
                                // if(this.state.is_teacher_ward == "false"){
                ...
                <div className="col-12">
                    <div className="form-row">
                        ...
                        <div className="col-6 form-group">
                         ...
                         <Link className="btn btn-info btn-md ml-2" to="/SuspensionalVoucher">Suspicious Amount</Link>
