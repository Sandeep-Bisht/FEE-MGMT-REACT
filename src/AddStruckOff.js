...
class AddStruckOff extends React.Component{
    constructor(props){
        super(props)
        this.state={
          AllStudent:[],
    ...
    StudentStrenght=async()=>{
     this.setState({AllStudent:[]})
       fetch("http://144:91:110:210:4800/StudentStrenghtForSos"
       ,{
           method: 'POST',
           headers: {
               Accept: 'application/json',
      ...
      if(item.student.tc_status=='0'){
          ...
          const url="http://144:91:110:210:4800/updateStruckOff"
                 ...
                 this.StudentStrenght()
                  ...
                  .then(err=>console.log(err))
      }else if(item.student.tc_status=='sos'){
        const data = new FormData()
        data.append('student_id',item.student._id) 
        data.append('academic_id',item._id)   
    ...
    render(){
        ...
        return(
            ...
            <div className= "row printCard printDefaulter">
                ...
                <div className="col-12 text-center">
                  <h4 className="pb-5">Struckoff</h4>
                </div>
                        ...
                        {this.state.AllStudent.sort((a,b) => parseInt(a.admission_no) - parseInt(b.admission_no)).map((item,index)=>{
                            return(
                              <tr>
                                ...
                                <td><Switch onChange={()=>{this.updateStruckOff(item)}} checked={item.student.tc_status=="0"?false:item.student.tc_status=="sos"?true:false} /></td>
                                {/* <td>{item.student.parent_address}</td> */}
                                {/* <button className="eidt"><i className="fa fa-edit"></i></button> */}
                              </tr>
                            )
