...
var StudentData = [];
var IdArray = [];
class StudentPromotionDemotion extends React.Component{
    constructor(props){
        super(props)
        this.state={
           AllStudent:[],
    ...
    await console.log("wait wait")
      const url = "http://localhost:4800/getStudentForUpgradeSingle"
      fetch(url
        , {
          method: 'POST',
          headers: {
    ...
    getToClass = async() => {
      await console.log("wait wait")
      const url = "http://localhost:4800/getClass"
      fetch(url
        , {
          method: 'POST',
          headers: {
  ...
  const url = "http://localhost:4800/StoreUpgradeStudent"
  fetch(url, {
          method: 'post',
          body: data
      })
  ...
  DeleteUpgradeStudent =async()=>{
    await console.log("wait")
    const data = new FormData()
    data.append('IdArray', JSON.stringify(IdArray))
    const url = "http://localhost:4800/DeleteUpgradeStudent"
    fetch(url, {
            method: 'post',
            body: data
        })
        .then(res => res.json())
        .then(data => {
            // alert("Upgraded Successfully Successfully")    
            this.getClass()   
            IdArray = [];         
        }).catch(err =>{
        });
...
export default StudentPromotionDemotion;